...
let
  ...
  cfgOpenSnitch = hasAttr "opensnitch" config.services;
...
in
{
  ...
  imports = [
    ...
    (mkRemovedOptionModule [ "programs" "firefox" "enableIcedTea" ]
      "Support for this option has been removed.")
    (mkAliasOptionModule [ "programs" "firefox" "extensions" ] [ "programs" "firefox" "nixExtensions" ])
  ];
  ...
  options = {
    programs.firefox = {
      ...
      # TODO: extraNativeMessagingHosts
      ...
      finalPackage = mkOption {
        ...
        default =
          let
            # FIXME: refactor it into options
            nativeMessagingHosts = pkgs.linkFarm "nativeMessagingHosts" [
              {
                name = "lib/mozilla/native-messaging-hosts/com.github.browserpass.native.json";
                path = pkgs.writeText "browserpass-native" (builtins.toJSON {
                  name = "com.github.browserpass.native";
                  description = "Browserpass native component for the Firefox extension";
                  path = lib.getBin pkgs.browserpass;
                  type = "stdio";
                  allowed_extensions = [ "nixos@browserpass-ce" ];
                });
              }

              {
                name = "lib/mozilla/native-messaging-hosts/com.samhh.bukubrow.json";
                path = pkgs.writeText "bukubrow-native" (builtins.toJSON {
                  name = "com.samhh.bukubrow";
                  description = "Bukubrow extension host application";
                  path = lib.getBin pkgs.bukubrow;
                  type = "stdio";
                  allowed_extensions = [ "nixos@bukubrow" ];
                });
              }

              {
                name = "lib/mozilla/native-messaging-hosts/passff.json";
                path = pkgs.writeText "passff-native" (builtins.toJSON {
                  name = "passff";
                  description = "Host for communicating with zx2c4 pass";
                  path = "${pkgs.passff-host}/share/passff-host/passff.py";
                  type = "stdio";
                  allowed_extensions = [ "nixos@passff" ];
                });
              }

              {
                name = "lib/mozilla/native-messaging-hosts/com.ugetdm.firefox.json";
                path = pkgs.writeText "uget-native" (builtins.toJSON {
                  name = "com.ugetdm.firefox";
                  description = "Integrate uGet with Mozilla Firefox";
                  path = lib.getBin pkgs.bukubrow;
                  type = "stdio";
                  allowed_extensions = [ "nixos@ugetintegration" ];
                });
              }
            ...
            package = pkgs.wrapFirefox cfg.package {
              inherit (cfg) extraPolicies extraPrefs nixExtensions;
              # NOTE: doesn't work for vendored extensions, since they change ids to nixos@extension
              extraNativeMessagingHosts = [ nativeMessagingHosts ];
              cfg = {
      ...
      nixExtensions = mkOption {
        ...
        description = ''
          ...
          Must be the result of pkgs.fetchFirefoxAddon
        '';
      };
  ...
  config = mkMerge [
    # FIXME: (mkIf (cfg.enable && cfgOpenSnitch) { services.opensnitch.allow = [ "${cfg.finalPackage}/lib/firefox/firefox" ]; })
    (mkIf cfg.enable {
      assertions = [
        (
          let
      ...
      home.packages =
        let
          ...
          # else
          #   (pkgs.wrapFirefox.override { config = bcfg; }) cfg.package { inherit (cfg) extraPolicies extraPrefs; nixExtensions = cfg.extensions; };
