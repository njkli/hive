...
let
  ...
  cfg = config.services.trezor-agent;
  ...
  identities_options = with types; { ... }: {
    options = {
      realName = mkOption { type = str; };
      emailAddress = mkOption { type = str; };
      comment = mkOption { default = null; type = nullOr str; };
      default = mkOption { default = false; type = bool; };
    };
  };
in
{
  ...
  options = with types; {
    services.trezor-agent = {
      ...
      identities = mkOption {
        type = attrsOf (listOf (submodule [ identities_options ]));
      };
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      home.file."${homedir}/gpg-agent.conf".text = concatStringsSep "\n"
        (optional (cfg.enableSshSupport) "enable-ssh-support"
          # GNUPGHOME=~/.gnupg/trezor trezor-gpg init -v --pin-entry-binary /nix/store/s49gnjpyp7w847xbinyrjf214ssna99h-pinentry-1.2.0-gnome3/bin/pinentry --passphrase-entry-binary /nix/store/s49gnjpyp7w847xbinyrjf214ssna99h-pinentry-1.2.0-gnome3/bin/pinentry --cache-expiry-seconds 60 "Johnny Bravo <johnny@bravo.com>"
          ++ [
